...
cat /kafka/config/server.properties.template | sed \
  -e "s|{{ZOOKEEPER_CONNECTION_STRING}}|zookeeper.service.consul:2181|g" \
  -e "s|{{ZOOKEEPER_CHROOT}}|${ZOOKEEPER_CHROOT:-}|g" \
  -e "s|{{KAFKA_BROKER_ID}}|${KAFKA_BROKER_ID:-0}|g" \
  -e "s|{{KAFKA_ADVERTISED_HOST_NAME}}|$HOST.node.consul|g" \
  -e "s|{{KAFKA_PORT}}|$PORT0|g" \
  -e "s|{{KAFKA_ADVERTISED_PORT}}|$PORT0|g" \
