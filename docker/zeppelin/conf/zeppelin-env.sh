...
#
export MASTER=mesos://${MESOS_MASTER}:5050
export ZEPPELIN_JAVA_OPTS="-Dspark.mesos.executor.home=/usr/lib/spark \
-Dspark.broadcast.factory=org.apache.spark.broadcast.HttpBroadcastFactory \
-Dspark.driver.port=${PORT2} \
-Dspark.fileserver.port=${PORT3} \
-Dspark.broadcast.port=${PORT4} \
-Dspark.blockManager.port=${PORT5} \
-Dspark.ui.port=${PORT6} \
-Dspark.replClassServer.port=${PORT7} \
"
# export ZEPPELIN_JAVA_OPTS="-Dspark.executor.uri=http://${MESOS_MASTER}/spark-1.4.1-bin-hadoop2.6.tgz"
export MESOS_NATIVE_LIBRARY=/usr/local/lib/libmesos.so
export MESOS_NATIVE_JAVA_LIBRARY=/usr/local/lib/libmesos.so

# export JAVA_HOME=
# export MASTER=mesos://${MESOS_MASTER}:5050                 # Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
...
export ZEPPELIN_LOG_DIR=/log        # Where log files are stored.  PWD by default.
